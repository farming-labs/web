...
- **`allowDeprecated`** – Allows execution of deprecated hooks if set to `true`.
...
- **`storage`** – Determines where hooks are stored (`"local"` for in-memory, `"redis"` for Redis storage).
- **`redisUrl`** – Specifies the Redis connection URL when `storage` is set to `"redis"`.
...
hook(hooks, "hook1", () => console.log("This is a Redis hook"));
```
...
```javascript
const {(currHook, removeHook)} = await hook(hooks, "hook1", ()=> console.log("This is a hook"), );
```
